...
float lightStrength(in vec2 intersection, in float cylLen, in float cylRadius, in vec3 worldNear, in vec3 rayDirection, in vec3 cylDirection) {
    ...
    return exp(-intersection.x * 0.01) * sin((1 - smoothstep(0, cylRadius, a)) * Pi / 2)
            * smoothstep(-0.1, 0, c)
...
void main() {
    ...
    fragColor = vec4(max(
            min(
                    vec3(0.95),
                    smoothstep(0, 3, ColorModulator.xyz * color.xyz * lightStrength(intersection, cylLen, LightRadius - 0.1, worldNear, rayDirection, cylDirection)) +
                            vec3(step(0.15, innerLightRadius) * smoothstep(0, 3, lightStrength(innerIntersection, cylLen, innerLightRadius, worldNear, rayDirection, cylDirection) - 0.2))
            ),
            // beam
            vec3(step(0.005, beamLightCylRadius) *
                    ...
                    (1 - smoothstep(5, 10, beamIntersection.x)))
